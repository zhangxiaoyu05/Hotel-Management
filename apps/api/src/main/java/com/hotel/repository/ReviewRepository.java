...
public interface ReviewRepository extends BaseMapper<Review> {
     ...
     * 计算评分质量统计
     */
    default List<Integer> calculateRatingDistribution(LocalDateTime startDate, LocalDateTime endDate) {
        // 这里应该使用SQL查询来计算评分分布
        // 为了简化，暂时返回模拟数据，实际实现需要在XML中编写SQL
        return Arrays.asList(5, 12, 25, 45, 30); // 1-5星评分分布
    }

    /**
     * 获取查询包装器
     */
    default QueryWrapper<Review> getQueryWrapper() {
