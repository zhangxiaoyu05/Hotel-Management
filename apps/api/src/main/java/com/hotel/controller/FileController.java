...
import com.hotel.util.ImageSecurityValidator;
...
public class FileController {
    ...
    private final ImageSecurityValidator imageSecurityValidator;
    ...
    private static final long MAX_FILE_SIZE_REVIEW = 5 * 1024 * 1024;    // 5MB
    ...
    private enum UploadType {
        ...
        REVIEW("review", MAX_FILE_SIZE_REVIEW, "5MB", false);
    ...
    public ResponseEntity<ApiResponse<UploadResponse>> uploadFile(
        ...
        try {
            ...
            if (ALLOWED_IMAGE_TYPES.contains(file.getContentType())) {
                ImageSecurityValidator.ValidationResult validationResult = imageSecurityValidator.validateImage(file);
                if (!validationResult.isValid()) {
                    log.warn("图片安全验证失败: {}", validationResult.getErrorMessage());
                    return ResponseEntity.badRequest()
            ...
            // 对图片进行压缩处理（仅对review类型）
            MultipartFile processedFile = file;
            if ("review".equals(uploadType.typeName) && ImageUtils.isSupportedImage(file)) {
                try {
                    byte[] compressedImageBytes = ImageUtils.compressImage(file);
                    processedFile = new MockMultipartFile(
                        file.getName(),
                        file.getOriginalFilename(),
                        file.getContentType(),
                        compressedImageBytes
                    );
                    log.info("图片压缩完成: {}, 原始大小: {}KB, 压缩后大小: {}KB",
                        originalFilename,
                        file.getSize() / 1024,
                        compressedImageBytes.length / 1024
                    );
                } catch (Exception e) {
                    log.warn("图片压缩失败，使用原始文件: {}, 错误: {}", originalFilename, e.getMessage());
                    // 压缩失败时使用原始文件
                }
            }

            // 上传文件
            UploadResponse uploadResponse = fileService.uploadFile(processedFile, uploadType.typeName);
